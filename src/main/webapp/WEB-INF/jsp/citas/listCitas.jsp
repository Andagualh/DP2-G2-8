		...
		<tbody>
			<c:forEach items="${selections}" var="cita">
				<tr>
					...
					<td><spring:url value="/pacientes/{pacienteId}"
							var="pacienteUrl">
							<spring:param name="pacienteId" value="${cita.paciente.id}" />
						</spring:url> <a href="${fn:escapeXml(pacienteUrl)}"><c:out
								value="${cita.paciente.nombre} ${cita.paciente.apellidos}" /></a></td>
					<td><c:choose>
							<c:when test="${cita.informe != null}">
								<spring:url value="{citaId}/informes/{informeId}"
									var="informeDetailsUrl">
					...
					<td><c:choose>
							<c:when test="${cita.fecha == dateOfToday || cita.fecha.isAfter(dateOfToday) && cita.informe == null}">
							...
							<c:otherwise>
								<c:out value="No puede borrarse"></c:out>
							</c:otherwise>
						</c:choose></td>
